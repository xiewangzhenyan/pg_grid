...
**非单调**（默认参数下约 51 µM 处到达峰值后回落）。这也是真值记绝对
浓度而不是"占最大浓度百分比"的原因：拐点绑定在绝对浓度上，相对刻度
既无法表达它，也无法让不同量程的图共用一条标定曲线。详见 Photophysics。
...
class Photophysics:
    ...
    最后一条是关键：默认参数下 F 在约 50 µM 处到达峰值后下降，同一灰度
    对应两个浓度。若真值只记百分比，这个拐点会随用户事后选的最大浓度
    漂移，标注就和图像里已经烘焙进去的物理对不上了；不同 C_max 的图也
    失去公共横轴，无法汇总成一条标定曲线。
    """
    ...
    path_length_cm: float = 0.30        # 孔内液层光程
